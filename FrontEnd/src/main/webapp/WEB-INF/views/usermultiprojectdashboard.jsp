...
<script type="text/javascript">
	$(document).ready(function() {
		$('#projects').change(function(event) {
			var name = document.getElementById('projects').value;
			$.get("getMasterPlan?projectName=" + name, function(data, status) {
						...
						<div class="box" style="height: 472px; overflow-y: auto;">
							...
							<table id="example1" class="table table-bordered table-striped">
								<tr>
									<td style="padding-right: 8px; width: 225px;">Select
										MasterPlan :</td>
									<td><select name="projects" id="projects">
											<option value="" selected="selected" disabled="disabled"
												hidden="true">Select the MasterPlan</option>
											<c:forEach items="${proList}" var="pro">
												<option value="${pro.name}">${pro.name} - Masterplan</option>
											</c:forEach>
									</select></td>
								</tr>
							</table>
							<div id="messagearea" style="display: none;">
								...
								<span style="color: green"> <b>Please upload the
										MasterPlan to proceed.</b></span> <a href="UploadMasterPlan">Upload
									Master Plan</a>
							</div>
